...
export function getFeature() {
	...
	const event = events.find(({ when, feature }) => {
		...
		if (
			now.getTime() <
			date.getTime() - feature.daysBefore * 24 * 60 * 60 * 1000
		) {
			return false;
		}
...
export function getOrderedEvents() {
	return events.sort((eventA, eventB) =>
		getComparativeDateForEvent(eventA) > getComparativeDateForEvent(eventB)
			? 1
			: -1
	);
}
...
export function getComparativeDateForEvent(event: IEvent) {
	...
	return (
		event.when.dates.find((date) => date > now) ||
		event.when.dates[event.when.dates.length - 1]
	);
}
